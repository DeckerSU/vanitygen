...
"Usage: %s [-vqnrik1NBT] [-t <threads>] [-f <filename>|-] [<pattern>...]\n"
...
main(int argc, char **argv)
{
	...
	while ((opt = getopt(argc, argv, "vqnrik1eE:P:NBTX:F:t:h?f:o:s:")) != -1) {
		switch (opt) {
		case 'v':
			verbose = 2;
			break;
		...
		case 'N':
			...
			break;
		case 'B':
			addrtype = 25; // from chainparams.cpp
			privtype = 63+128;
			break;
		case 'T':
			addrtype = 111;
